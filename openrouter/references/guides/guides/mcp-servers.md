...
The following example uses Anthropic's MCP client SDK to interact with the File System MCP, all with OpenRouter under the hood.

> **Note**: Interacting with MCP servers is more complex than calling a REST endpoint, as the MCP protocol is stateful and requires session management.
...
A helper function transforms MCP tool schemas into OpenAI-compatible format:
...
The main client class handles connection to MCP servers, tool listing and availability, query processing with the OpenRouter API, tool execution and result handling, and multi-turn conversation management.
...
- [OpenRouter Python SDK](/docs/sdks/python) - Python SDK documentation
- [OpenRouter TypeScript SDK](/docs/sdks/typescript) - TypeScript SDK documentation
- [Provider Routing](/docs/features/provider-routing) - Configure provider routing
